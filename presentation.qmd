---
title: "AI Integration with the Claude API"
author: "Shubhan Tamhane "
format: revealjs
theme: solarized
execute: 
  echo: false

---

## Introduction

 **Overview of AI in Data Science**

- AI is increasingly being used in data science for automating repetitive tasks, analyzing large datasets, and extracting meaningful insights
- AI models like **Claude** are spearheading the new wave to improve accuracy
    and efficiency surrounding data science tasks 
- Claude API Overview
  - Claude is a LLM developed by Anthropic 

---

## Why this matters
> Imagine if you could add a brain to any app with just 10 lines of code

- The Claude API makes AI Integration simple 
- No ML knowledge required

---

## Step 1: Keeping your key safe
- The first step to start building with Claude is to get an API key from
  https://console.anthropic.com/
- **Never** hardcode your API key directly in your code
```{python}
#| eval: false
#| echo: true
# ❌ DO NOT do this — exposes your key if pushed to GitHub
client = anthropic.Anthropic(api_key="sk-ant-abc123...")
```

**Instead, store it in a `.env` file:**
```{bash}
# .env
ANTHROPIC_API_KEY=sk-ant-your-key-here
```

---

## Then load it in Python:
```{python}
#| eval: false
#| echo: true
# ✅ Do this instead
from dotenv import load_dotenv
import os

load_dotenv()

client = anthropic.Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))
```

---

## How the API Works

- The core idea is very simple. You send a message, and you get a response
```{python}
#| eval: false
#| echo: true
from anthropic import Anthropic
client = Anthropic()

message = client.messages.create(
    model="claude-sonnet-4-5-20250929",
    max_tokens=1024,
    messages=[
        {"role": "user", "content": "Hello Claude!"}
    ]
)

print(message.content[0].text)
```

---

## The Input 

::: {style="font-size: 0.7em"}
| Parameter | Required | What it does |
|-----------|----------|--------------|
| `model` | ✅ | Which Claude to use — Haiku, Sonnet, or Opus |
| `max_tokens` | ✅ | Max length of the response — API won't go beyond this |
| `system` | ❌ | Sets Claude's personality/behavior for the whole conversation |
| `messages` | ✅ | The conversation — must always include at least one `user` message |
|`temperature` |  ❌ | Creativity of response|
:::

---

## Choosing the Right Model

::: {style="font-size: 1.4em; margin-top: 1em"}
| Model   | Best For               | Speed  | Cost |
|---------|------------------------|--------|------|
| Haiku   | Simple, fast tasks     | Fast   | \$    |
| Sonnet  | Balanced everyday use  | Medium | \$\$   |
| Opus    | Complex reasoning      | Slower | \$\$\$ |
:::

---

## What Are Tokens?

- Tokens are the basic units the API reads and writes — not words, not characters, but chunks of text
- As a rough rule: **1 token ≈ 4 characters** 

::: {style="font-size: 0.85em; margin-top: 0.5em"}
| Text | Tokens |
|------|--------|
| `"Hello"` | 1 token |
| `"Hello, how are you?"` | 5 tokens |
:::

- `max_tokens` controls how long Claude's response can be
- You are billed for input tokens (what you send) + output tokens (what Claude returns)



---

## System Prompts
A system prompt allows you to give a role to Claude and shape how Claude behaves across a whole conversation
```{python}
#| eval: false
#| echo: true
message = client.messages.create(
    model="claude-sonnet-4-5-20250929",
    max_tokens=1024,
    system="You are a helpful teaching assistant who explains things simply.",
    messages=[
        {"role": "user", "content": "What is an API?"}
    ]
)
```

## System Prompt does it really matter?
:::: {.columns}

::: {.column width="50%"}
**Without a system prompt**

*Prompt: "Explain APIs"*
```
An API (Application Programming
Interface) is a set of rules that
allows different software
applications to communicate with
each other. It defines the methods
and data formats that applications
can use to request and exchange
information...
```
:::

::: {.column width="50%"}
**With a system prompt**

*System: "You explain things to a 10 year old"*

*Prompt: "Explain APIs"*
```
Imagine you're at a restaurant. You
don't go into the kitchen to make
your own food — you just tell the
waiter what you want, and they bring
it back. An API is like that waiter!
It lets apps talk to each other
without needing to know how the
other one works inside 🍕
```
:::

::::


---

## Temperature 

- Temperature controls how **creative vs predictable** Claude's responses are
- Ranges from `0.0` to `1.0`

::: {style="font-size: 0.85em; margin-top: 0.5em"}
| Temperature | Behavior | Best For |
|-------------|----------|----------|
| `0.0` | Predictable, focused | Data extraction, Q&A |
| `0.5` | Balanced | General use |
| `1.0` | Creative, varied | Brainstorming, writing |
:::
```{python}
#| eval: false
#| echo: true
message = client.messages.create(
    model="claude-sonnet-4-5-20250929",
    max_tokens=1024,
    temperature=0.9,   # ← add this
    messages=[
        {"role": "user", "content": "Write me a tagline for a coffee shop"}
    ]
)
```

----

## Does Temperature Actually Matter?

:::: {.columns}

::: {.column width="50%"}
**temperature=0.0**

*Prompt: "Write a tagline for a coffee shop"*
```
Quality coffee, served fresh daily.
```
*Same answer every single time.*
:::

::: {.column width="50%"}
**temperature=1.0**

*Prompt: "Write a tagline for a coffee shop"*
```
Where every sip tells a story ☕

Fuel your chaos, one cup at a time.

Life's too short for bad coffee
and boring mornings.
```
*Different answer every time.*
:::

::::

---


## The Output
```{python}
#| eval: false
#| echo: true
Message(
    id='msg_01XFDUDYJgAACzvnptvVoYEL',
    content=[
        TextBlock(
            text='An API is a way for two applications
                  to talk to each other...',
            type='text'
        )
    ],
    model='claude-sonnet-4-5-20250929',
    role='assistant',
    stop_reason='end_turn',
    usage=Usage(input_tokens=15, output_tokens=120)
)
```

---

## Multi Turn Conversations
Pass the full message history to maintain context.
```{python}
#| eval: false
#| echo: true
messages = [
    {"role": "user", "content": "My name is Alex."},
    {"role": "assistant", "content": "Nice to meet you, Alex!"},
    {"role": "user", "content": "What's my name?"}
]

response = client.messages.create(
    model="claude-sonnet-4-5-20250929",
    max_tokens=512,
    messages=messages
)
```

---

## Demo: Building a Sentiment Analyzer

**What we're building:** A tool that analyzes the sentiment of text (positive, negative, neutral)

::: {style="font-size: 0.75em"}
```{python}
#| eval: false
#| echo: true
from anthropic import Anthropic
import os

client = Anthropic()

def analyze_sentiment(text):
    message = client.messages.create(
        model="claude-sonnet-4-5-20250929",
        max_tokens=100,
        temperature=0,  # We want consistent results
        system="You are a sentiment analyzer. Respond with only: positive, negative, or neutral.",
        messages=[
            {"role": "user", "content": f"Analyze the sentiment: {text}"}
        ]
    )
    return message.content[0].text

# Test it
review = "This product completely changed my life! Best purchase ever."
print(analyze_sentiment(review))
```
:::

---

## Running the Demo

**Input:**
```
"This product completely changed my life! Best purchase ever."
```

**Output:**
```
positive
```

**Why this works:**

- System prompt constrains the output to just 3 options
- `temperature=0` ensures consistent classification
- `max_tokens=100` keeps it fast and cheap


---

## Making It Better: Structured Output

::: {style="font-size: 0.7em"}
```{python}
#| eval: false
#| echo: true
system_prompt = """You are a sentiment analyzer. 
Return your response as JSON with this format:
{"sentiment": "positive/negative/neutral", "confidence": 0-100, "reason": "brief explanation"}"""

message = client.messages.create(
    model="claude-sonnet-4-5-20250929",
    max_tokens=200,
    temperature=0,
    system=system_prompt,
    messages=[
        {"role": "user", "content": f"Analyze: {review}"}
    ]
)

import json
result = json.loads(message.content[0].text)
print(f"Sentiment: {result['sentiment']} ({result['confidence']}% confident)")
print(f"Reason: {result['reason']}")
```

**Output:**
```
Sentiment: positive (95% confident)
Reason: Enthusiastic language and strong positive words like "changed my life" and "best purchase ever"
```
:::

---


## Retrieval-Augmented Generation (RAG)

**The Problem:** Claude only knows what's in its training data, it doesn't know about your company docs, your research papers, or your database.

**The Solution:** RAG = giving Claude access to your own data at query time

::: {style="font-size: 0.85em"}

1. Store your data in a searchable format (vector database, search index, etc.)
2. Retrieve relevant chunks based on the user's question
3. Send those chunks to Claude along with the user's query

**Example use case:**
- Chatbot that answers questions about your company's internal docs
:::

---

## Resources & Next Steps

::: {style="font-size: 0.9em"}
**Official Documentation**

- [Anthropic API Docs](https://docs.anthropic.com) — Full API reference
- [Prompt Engineering Guide](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview) — Best practices for writing prompts
- [Claude Console](https://console.anthropic.com) — Get your API key and manage usage

**Learning Resources**

- [Anthropic Cookbook](https://github.com/anthropics/anthropic-cookbook) — Code examples and tutorials
- [API Reference](https://docs.anthropic.com/en/api) — Detailed endpoint documentation


:::





